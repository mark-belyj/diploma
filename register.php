...
//Проверяем нет ли уже такого адреса в БД.
                $result_query = $mysqli->query("SELECT `profile_login` FROM `profile` WHERE `profile_login`='".$email."'");
...
//Запрос на добавления пользователя в БД
        $result_query_insert = $mysqli->query("INSERT INTO `profile` (profile_login, profile_pass, profile_date_registration) VALUES ('".$email."', '".$password."', NOW())");
        $query_id = $mysqli->query("SELECT profile.`profile_id`  FROM profile  WHERE profile.`profile_id` = LAST_INSERT_ID()");
        if (($row = $query_id->fetch_assoc()) != false) { //Если ошибок в запросе нет
            $profile_id = $row['profile_id'];
        }
        $result_query_insert2 = $mysqli->query("INSERT INTO `info` (profile_id, info_first_name, info_last_name) VALUES ('".$profile_id."', '".$first_name."', '".$last_name."')");
        $result_query_insert3 = $mysqli->query("INSERT INTO `geolocation` (profile_id, geolocation_marker_desc) VALUES ('".$profile_id."', '".$first_name."')");
